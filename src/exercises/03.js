...
function ToggleConsumer(props) {
  return <ToggleContext.Consumer>{context => {
    if (!context) {
...
// in some situatioins, making a default calue for context would make sense instead of creating the above ToggleConsumer()
...
class Toggle extends React.Component {
  ...
  // ToggleConsumer and rather than getting `on` and `toggle`
  // from props, it'll get it from the ToggleConsumer value.
  static On = ({children}) => (
    <ToggleConsumer>
      {({on}) => (on ? children : null)}
    </ToggleConsumer>
